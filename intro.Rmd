...
tidyverse의 패키지는 상당히 자주 변경된다. <코드체>tidyverse_update()</코드체>를 실행하여 업데이트가 가능한지 확인할 수 있다.
...
구글 검색이 도움이 되지 않으면 [stackoverflow](http://stackoverflow.com)에 가 보라. 기존 답변을 검색하는 데 시간을 조금 투자해 보라. <코드체>[R]</코드체>을 포함하면 R 사용과 관련된 질문과 답변으로 검색을 제한한다. 유용한 정보가 없으면 최소한의 재현 가능한 예 (reproducible example), 줄임말로 **reprex**를 준비하라. 좋은 reprex는 다른 사람들이 여러분을 도울 수 있게 해 주며, 이를 만드는 과정에서 문제의 답을 스스로 찾는 경우가 많다. 
...
1.  질문에 <볼드>데이터</볼드>를 포함시키는 가장 쉬운 방법은 <코드체>dput()</코드체> 을 사용하여 재현되는 R 코드를 생성하는 것이다. 예를 들어, R에서 <코드체>mtcars</코드체> 데이터셋을 다시 만들려면 다음 단계로 수행하라.  

    1. R 에서 <코드체>dput(mtcars)</코드체>를 실행한다.  
    1. 출력을 복사한다.
    1. 재현가능한 스크립트에서, <코드체>mtcars <-</코드체> 를 입력하고 붙여 넣는다.  
